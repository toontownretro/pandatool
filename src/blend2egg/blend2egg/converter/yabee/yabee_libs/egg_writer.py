...
class Group:
    ...
    def get_full_egg_str_arr(self, level=0):
        ...
        egg_str = []
        if self.object:
            if self.object.__class__ == bpy.types.Bone or self.object.type != 'ARMATURE':
                if self.object.__class__ == bpy.types.Bone:
                    ...
                    if self.object.type == 'EMPTY' and self.object.name.startswith('lod'):
                        # Found an LOD switch.
                        near_distance = self.object.get('near_distance')
                        if near_distance is None:
                            print("WARNING: LOD switch %s does not have a near_distance property!  Switch will not export." % self.object.name)
                        far_distance = self.object.get('far_distance')
                        if far_distance is None:
                            print("WARNING: LOD switch %s does not have a far_distance property!  Switch will not export." % self.object.name)

                        if near_distance is not None and far_distance is not None:
                            egg_str.append('%s<SwitchCondition> {\n' % ('  ' * (level + 1)))
                ...
                for ch in self.children:
                    egg_str.append(ch.get_full_egg_str(level))
        else:
            if ANIM_TYPE == 'model':
                egg_str.append('%s<Group> %s {\n' % ('  ' * level, eggSafeName(CHAR_NAME)))
                egg_str.append('%s<Dart> { 1 }\n' % ('  ' * (level + 1)))
            for ch in self.children:
                egg_str.append(ch.get_full_egg_str(level + 1))
            if ANIM_TYPE == 'model':
...
class AnimCollector:
    ...
    def __init__(self, obj_list, start_f, stop_f, framerate, name, action=None):
        ...
        for obj in obj_list:
            if obj.__class__ != bpy.types.Bone:
                ...
                elif obj.type == 'ARMATURE' and not gotArmature:
                    if obj.proxy:
                        obj = obj.proxy
                    gotArmature = True
                    print("Got armature", obj)
    ...
    def get_skeleton_anim_str(self, obj_name):
        ...
        skel_str = ''
        data = self.obj_anim_ref[obj_name]
        #print("obj anim ref[%s]" % obj_name, data)
    ...
    def get_full_egg_str(self):
        ...
        if self.obj_anim_ref:
            egg_str += '<Table> {\n'
            egg_str += '  <Bundle> %s {\n' % eggSafeName(CHAR_NAME)
            for obj_name, _ in self.obj_anim_ref.items():
...
#                           WRITE OUT
# -----------------------------------------------------------------------
def write_out(fname, anim, anim_type,
              tbs, autoselect,
              apply_obj_transform, m_actor, apply_m, apply_coll_tag,
              loop_normals, force_export_vertex_colors, char_name, coordinate_system, objects=None):
    global FILE_PATH, ANIMATION, \
    ...
    IS_ANIM_EXPORT = anim_type == 'chan'
    ANIM_TYPE = anim_type
    COORDINATE_SYSTEM = coordinate_system

    CHAR_NAME = char_name
    if not CHAR_NAME:
        # If character name not specified, take it from the filename.
        # This convention is taken from the Maya exporter.
        CHAR_NAME = os.path.splitext(os.path.basename(fname))[0]
    s_acc = '%.6f'
    ...
    try:
        ...
        if not errors:
            ...
            if not IS_ANIM_EXPORT:
                print("Writing a model.")
                # No animation, we're writing the model.
                ...
                print("Writing an animation.")

                if len(bpy.data.actions) == 0:
                    exit("ERROR: Animation export was requested, but the blend file doesn't contain an action!")

                action = bpy.data.actions[0]
                start_frame, end_frame, fps = ANIMATION
                ...
                ac = AnimCollector(obj_list, start_frame, end_frame,
                ...
                if len(a_egg_str) > 0:
                    ...
                    if not COORDINATE_SYSTEM:
                        ...
                        up_axis = COORDINATE_SYSTEM.upper()
                    a_file.write('<CoordinateSystem> { %s-up } \n' % up_axis)
