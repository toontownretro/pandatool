
#define INSTALL_SCRIPTS \
  blend2egg.py \
  blend2egg/converter/__init__.py \
  blend2egg/converter/blend2egg_converter.py \
  blend2egg/converter/blender_script.py \
  blend2egg/converter/yabee/yabee.py \
  blend2egg/converter/yabee/__init__.py \
  blend2egg/converter/yabee/yabee_libs/__init__.py \
  blend2egg/converter/yabee/yabee_libs/egg_writer.py \
  blend2egg/converter/yabee/yabee_libs/utils.py \
  blend2egg/__init__.py \
  blend2egg/blender_script_common.py \
  blend2egg/blenderutils.py \
  blend2egg/cli.py \
  blend2egg/common.py
