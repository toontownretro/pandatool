...
scan_texture(const Filename &filename, LVecBase4d &geometry) {
  PT(Material) mat = Material::load(filename);
  if (mat == nullptr) {
    nout << "Unable to read material " << filename << "\n";
    return false;
  }
...
run() {
  ...
  for (ti = _texture_names.begin(); ti != _texture_names.end(); ++ti) {
    ...
    LVecBase4d geometry;
    bool texture_ok = scan_texture(filename, geometry);
    if (!texture_ok) {
      all_ok = false;
    }
    ...
    EggMaterial *tref = new EggMaterial(name, filename);
